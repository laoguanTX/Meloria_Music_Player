...
class _LibraryStatsScreenState extends State<LibraryStatsScreen> {
  ...
  void initState() {
    ...
    _libraryStatsFuture =
        Provider.of<MusicProvider>(context, listen: false).getLibraryStats();
  }
          ...
          builder: (context, snapshot) {
            ...
            return Consumer<MusicProvider>(
              builder: (context, musicProvider, child) {
                final totalSongsFromDb =
                    initialDbStats['total'] ?? 0; // Use DB count from future
                final livePlaylistsCount = musicProvider.playlists.length;
                final uniqueAlbumsCount =
                    musicProvider.getUniqueAlbums().length;
                final uniqueArtistsCount =
                    musicProvider.getUniqueArtists().length;
                final totalDurationOfSongs =
                    musicProvider.getTotalDurationOfSongs();
                final mostPlayedSongs =
                    musicProvider.getMostPlayedSongs(count: 3);
                ...
                return ListView(
                  children: <Widget>[
                    ListTile(
                      ...
                      trailing: Text('$totalSongsFromDb 首',
                          style: Theme.of(context).textTheme.titleMedium),
                    ),
                    ListTile(
                      leading: const Icon(Icons.album),
                      title: const Text('总专辑数量'),
                      trailing: Text('$uniqueAlbumsCount 张',
                          style: Theme.of(context).textTheme.titleMedium),
                    ),
                    ListTile(
                      leading: const Icon(Icons.person),
                      title: const Text('总艺术家数量'),
                      trailing: Text('$uniqueArtistsCount 位',
                          style: Theme.of(context).textTheme.titleMedium),
                    ),
                    ListTile(
                      leading: const Icon(Icons.playlist_play),
                      title: const Text('播放列表数量'),
                      trailing: Text('$livePlaylistsCount 个',
                          style: Theme.of(context).textTheme.titleMedium),
                    ),
                    const Divider(),
                    ListTile(
                      ...
                      trailing: Text(_formatDuration(totalDurationOfSongs),
                          style: Theme.of(context).textTheme.titleMedium),
                    ),
                    const Divider(),
                    const Padding(
                                ...
                                style: Theme.of(context)
                                    ...
                                    ?.copyWith(
                                        fontSize: Theme.of(context)
                                            .textTheme
                                            .bodyMedium
                                            ?.fontSize)), // Adjusted for trailing context
                          )),
                    const Divider(),
                    const Padding(
                      padding: EdgeInsets.symmetric(vertical: 8.0),
